...
if ( is_admin() ){ // admin actions
	...
	$config = array(
			...
			'api_url' => 'https://api.github.com/repos/hashcore/wordpress-plugin', // the github API url of your github repo
			'raw_url' => 'https://raw.github.com/hashcore/wordpress-plugin/master', // the github raw url of your github repo
			'github_url' => 'https://github.com/hashcore/wordpress-plugin', // the github url of your github repo
			'zip_url' => 'https://github.com/hashcore/wordpress-plugin/zipball/master', // the zip url of the github repo
...
function fc_settings_page() {
	...
	//publisher URL variables
	$publisher_field_url = 'hc_input_url';	
    $hc_publisher_url = 'hc_publisher_url';
	$hc_url = get_option( $hc_publisher_url );
...
if( isset($_POST[ $hidden_field_name ]) && $_POST[ $hidden_field_name ] == 'Y' )
{
	...
	$hc_url = $_POST[$publisher_field_url]; 
	update_option( $hc_publisher_url, $hc_url );
...
<table>
	...
	<tr>
		...
		<td>
			...
			You can find your id from this website: http://jelled.com/instagram/lookup-user-id
		</td>
	</tr>
	<tr>
		<td><?php _e('Publisher URL', 'fc-menu' ); ?></td>
		<td>
			<input type="text" name="<?php echo $publisher_field_url; ?>" value="<?php echo $hc_url; ?>" />
			&nbsp;Please enter your domain excluding the http and/or www, for example, domain.com or sub.domain.com which you registered with.
		</td>
	</tr>
	...
	<tr>
		<td><?php _e('Recommended by Field', 'fc-menu' ); ?></td>
		<td>
...
<h1><a target="_blank" href="http://dashboard.hashcore.com/reports.php?pub=<?php echo get_option('hc_publisher_url'); ?>&code=<?php echo get_option('hc_publisher_text'); ?>">Go to reporting dashboard</a></h1>
<?php }
...
// Display footer
      function fc_text_inputreal() 
      {
		echo '<script>';
			echo '(function(w,d){';
				echo 'w.puburl = "'.get_option('hc_publisher_url').'";';
				echo 'w.pubcode = "'.get_option('hc_publisher_text').'";';
				echo 'w.instagram = "'.get_option('instagram_input_text').'"; ';
				echo 'w.twitter = "'.get_option('fc_input_text').'";';
				echo 'var s=document.createElement("script"); s.async=true; s.src="http://c.hashcore.com/api/widget.js?r=" + Math.round(Math.random(1000,5000));';
				echo 'var s2=document.getElementsByTagName("script")[0]; s2.parentNode.insertBefore(s,s2);';
...
function fc_publisher_url()
{
	if(get_option('hc_publisher_url') == '')
	{
		return "x";
	}
	else
	{
		return get_option('hc_publisher_url');
	}
}
...
{		
	...
	return $content . "<div id=\"hashcore-widget-container\"><h1><span  style=\"float:left\">".$title."</span> <span style=\"float:right\"><a class=\"hashcore-recommendby-link\" href=\"http://hashcore.com\" target=\"_blank\">".$recommend." Hashcore</a></span><br style=\"clear:both;\" /></h1></div>";
